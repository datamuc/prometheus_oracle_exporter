...
// adapted from to https://github.com/QubitProducts/exporter_exporter/blob/master/service_windows.go
...
func manageService() {
	const winSvcName = "prometheus_oracle_exporter"
	...
	case "install":
		err = installService(winSvcName, "Prometheus exporters for Oracle databases")
	case "uninstall":
...
func installService(name, desc string) error {
	...
	flag.Visit(func(f *flag.Flag) {
		if f.Name != "winsvc" {
			serviceArgs = append(serviceArgs, fmt.Sprintf("-%s=%s", f.Name,f.Value.String()))
			//serviceArgs = append(serviceArgs, f.Value.String())
		}
	})
	if len(serviceArgs) <= 0 {
		serviceArgs = append(serviceArgs, "-configfile", fmt.Sprintf("%s\\oracle.conf", filepath.Dir(exepath)))
	}
	m, err := mgr.Connect()
	if err != nil {
...
func runService(name string) {
	...
	err := svc.Run(name, &exporterService{})
	if err != nil {
